...
if [ -n "$COMPOSER_DRUPAL_LENIENT" ]; then
    ...
    cd "${APP_ROOT}" && time composer -n require "$COMPOSER_DRUPAL_LENIENT" --no-install
fi
# Add the project to composer (it will get the version according to the branch under `/repo/name_of_project`)
cd "${APP_ROOT}" && time composer -n require drupal/"$DP_PROJECT_NAME" --no-interaction --no-install
